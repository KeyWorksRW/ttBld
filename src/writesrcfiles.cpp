...
const char* sfopt::txtNinjaVerFormat = "# Requires MakeNinja version %d.%d.%d or higher to process";
...
bool CWriteSrcFiles::WriteUpdates(const char* pszFile)
{
    ...
    {
        bWarned = true;
        int major = 1;     // these three get filled in to the minum MakeNinja version required to process
        int minor = 0;
        int sub   = 0;
    ...
    while (kfIn.ReadLine())
    {
        if (ttIsSameSubStrI(kfIn, "# Requires MakeNinja"))
        {
            bSeenVersion = true;
            CVerMakeNinja verinfo;
