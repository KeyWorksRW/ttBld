    ...
    while (kf.ReadLine())
    {
        ...
        switch (m_section)
        {
            ...
            case SECTION_OPTIONS:
            {
                ...
                if (aOptions[pos])
                {
                    ...
                    if (pszFile)
                    {
                        ...
                        if (*pszFile)
                        {
                            // [KeyWorksRW - 11-29-2018] I don't believe the HH compiler from MS supports comments after
                            // filenames, but we can't be certain other compilers and/or authoring systems don't use
                            // them -- so remove any comment just to be sure.
            ...
            case SECTION_TEXT_POPUPS:
            {
                ...
                // This section may include .h files that are parsed, but not compiled, but we need them for dependency
                // checking.
...
{
    /*
       The problem we need to solve with this function is that filename locations in the .hhp file will be relative to
       the .hhp file -- which in turn can be in a different location. So for example, you might have the following:
    ...
    */
    ...
    {
        // If we're in a nested .HHP file, then we need to first get the location of the nested .HHP, use that to get
        // the location of the pszFile;

        ttConvertToRelative(m_cszRoot, pszHHP, cszHHP);
        cszHHP.FullPathName();  // now that we have the location relative to our original .hhp file, convert it to a
                                // full path
...
bool CNinja::CreateHelpFile()
{
    ...
    file.printf("# WARNING: THIS FILE IS AUTO-GENERATED by %s. CHANGES YOU MAKE WILL BE LOST IF IT IS AUTO-GENERATED "
                "AGAIN.\n\n",
                txtVersion);
    ...
    if (chhp.m_lstDependencies.GetCount() > 0)
    {
        ...
        file.printf("  %s\n", chhp.m_lstDependencies[chhp.m_lstDependencies.GetCount() -
                                                     1]);  // last one doesn't have trailing '$' character
    }
    else
    {
        file.printf("build %s : compile %s\n", (char*) m_cszChmFile, GetHHPName());
